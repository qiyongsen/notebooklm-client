...
describe('paths', () => {
  ...
  it('should respect NOTEBOOKLM_HOME env var', () => {
    ...
    expect(getSessionPath()).toBe('/tmp/nb-test-env/session.json');
    expect(getProfileDir()).toBe('/tmp/nb-test-env/chrome-profile');
    expect(getRpcIdsPath()).toBe('/tmp/nb-test-env/rpc-ids.json');
  });
  ...
  it('should respect setHomeDir override over env var', () => {
    ...
    expect(getSessionPath()).toBe('/tmp/override-dir/session.json');
  });
});
